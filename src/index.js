...
(function (root, factory) {
	if (typeof define === "function" && define.amd) {
		...
		define([
			...
			"@cocreate/indexeddb",
			"@cocreate/config"
		], function (uuid, indexeddb, config) {
			return factory(
				true,
				WebSocket,
				uuid,
		...
		root.returnExports = factory(
			...
			WebSocket,
			root["@cocreate/uuid"],
			root["@cocreate/indexeddb"],
			root["@cocreate/config"]
		);
	}
})(
	typeof self !== "undefined" ? self : this,
	function (isBrowser, WebSocket, uuid, indexeddb, config) {
		...
		const CoCreateSocketClient = {
			...
			organization_id: async () => {
				return (
					organizationPromise ||
					(organizationPromise = getOrganization())
				);
			},
			...
			async init() {
				function defaultHost() {
					...
					if (host.startsWith("127.0.0.1")) host = "cocreate.app";
					return host;
				}
				const defaults = {
					clientId: indexeddb.ObjectId().toString(),
					host: defaultHost()
				};
				const keys = [
					"clientId",
			...
			delete(key) {
				let socket;
				if (typeof key === "string") socket = this.get(key);
				if (!socket || (!socket.id && !socket.url)) return;
			...
			async create(data = {}) {
				...
				if (this.organization === "canceled") return;
				...
				for (let url of urls) {
					...
					try {
						...
						if (!socketId) {
							...
							if (isBrowser)
								socketId = sessionStorage.setItem(
									"socketId",
									socketId
								);
							// else
					...
					socket.onmessage = function (message) {
						try {
							...
							if (
								message.broadcastClient &&
								message.broadcastBrowser !== false &&
								isBrowser &&
								message.broadcastBrowser &&
			...
			__fireListeners(data) {
				...
				if (data.method.includes(".")) {
					const listeners =
						this.listeners.get(data.method.split(".")[0]) || [];
					listeners.forEach((listener) => {
						listener(data);
					});
				}
			...
			async getUrls(data = {}) {
				...
				if (isBrowser) {
					...
					} else if (location.protocol !== "https:") protocol = "ws";
				}
				...
				if (typeof host === "string") {
					...
					for (let i = 0; i < host.length; i++) {
						...
						if (host[i].includes("://")) url = `${host[i]}`;
						else url = `${protocol}://${host[i]}`;
						...
						if (balancer == "mesh") urls.push(url);
						else {
							let socket = this.get(url);
							if (socket !== false) {
								urls.push(url);
								break;
							} else hostUrls.push(url);
						}
					}
